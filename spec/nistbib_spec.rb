RSpec.describe NistBib do
  ...
  it 'return xml of hit' do
    VCR.use_cassette '8011' do
      ...
      expect(hits.first.to_xml).to be_equivalent_to File.read(file_path).sub(/2019-02-05/, Date.today.to_s)
    end
  end
  ...
  it 'get a code' do
    VCR.use_cassette '8200' do
      ...
      expect(result).to be_equivalent_to File.read(file_path).sub(/2019-02-06/, Date.today.to_s)
    end
  end
  ...
  it 'get DRAFT' do
    VCR.use_cassette 'draft' do
      ...
      File.write file_path, result unless File.exist? file_path
      expect(result).to be_equivalent_to File.read(file_path).gsub(/2019-02-06/, Date.today.to_s)
    end
  end
